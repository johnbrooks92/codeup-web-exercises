...
var fruits = ['tomato', 'strawberry', 'mango', 'kiwi', 'guava', 'banana']
var veggies = ['broccoli', 'carrot', 'cauliflower', 'eggplant', 'Tomato', 'zucchini']
...
function hasVowels(stringx) {

    const vowels = ["a", "e", "i", "o", "u", "A", "E", "I", "O", "U"]

    var counter = 0;
    for (let i = 0; i<stringx.length; i++){
        console.log(stringx[i])
        if (vowels.includes(stringx[i])){
            counter+=1
        }
    }
    return counter>0
...
function startsWithVowel(x) {
    let fstLetter = x.charAt(0).toLowerCase();
...
function endsWithVowel(x) {
    let lstLetter = x.charAt(x.length-1).toLowerCase();
...
function startsAndEndsWithVowel(i) {
    if (startsWithVowel(i) && endsWithVowel(i))
    {
        return true;
    }
    return false;
...
function shortestString(x) {
    var shortest = "qwertyuiopasdfghjklzxcvbnm";
    for(var i = 0; i < x.length; i++) {
...
function numberOfItemTypes(x) {
    ...
    for(var i = 0; i < x.items.length; i++) {
        if(uniqueItems.includes(x.items[i].title)) {
            continue;
        }
        uniqueItems.push(x.items[i].title);
    }
...
function mostSpentOnItem(x){
    ...
    for(var i=0;i<x.items.length;i++) {
        if((x.items[i].price * x.items[i].quantity) > (mostSpent.price * mostSpent.quantity)) {
            mostSpent = x.items[i];
        }
    }
    return mostSpent;
